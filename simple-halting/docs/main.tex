...
The halting problem is a well known problem in computability theory. In 1936, Alan Turing proved that an algorithm to solve every input pair can not exist. More precisely, this problem can be defined as a decision problem and determines if a program will continue running or not based on a generic input pair. Formally, the problem can be defined as follows. Suppose a problem \texttt{$Halt_{TM}$} Turing Machine A that determines if a given machine description halts 
