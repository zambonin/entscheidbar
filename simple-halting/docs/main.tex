...
The halting problem is a well known problem in computability theory. In 1936, Alan Turing proved that an algorithm to solve every input pair can not exist. More precisely, this problem can be defined as a decision problem and determines if a program will continue running or not based on a generic input pair. Formally, the problem can be defined as follows. Suppose a problem $Halt_{TM} = {<M, w> | Turing Machine M halts with w}$ and a machine $H$ that solves (decides) $Halt_{TM}$. Now we introduce a new machine $B$ that takes $M$ and runs $H$ with $<M, <M>>$, and halts if $H$ rejects.
