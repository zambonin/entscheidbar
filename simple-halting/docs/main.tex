...
Identifying state repetitions is a simpler approach, since it needs not to look at all the possible states for a program. Since it cannot have more than $1000$ lines, the only instruction that can alter this behaviour semantically is \texttt{CALL}, by means of its looping capabilities. Hence, infinite loops only happen when two states are equal exactly when those instructions are executed. Thus, it is only necessary to check whether the program configuration is equal between two \texttt{CALL} instructions. 

With this information in mind, we need only to define what is such a configuration in the context of SHP. Recall that no additional information on the depth level or line number is needed. Then, we define the ``memory state'', that is, the sequence of registers \texttt{R0} to \texttt{R9}, as the configuration. Ergo, it is always possible to know whether a program will halt or not, and consequently, SHP is decidable.
