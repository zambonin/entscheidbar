...
The most complex behaviour of the code, however, is met when parsing instructions that enter and exit recursion states. A optimization is added here, that features memoization of previously calculated recursion states. This allows the code to pass the $\frac{1}{8}s$ time target, since programs that calculate higher Fibonacci numbers will take a long time to execute if its  are not memoized, a commonly known pitfall for recursive calculation of Fibonacci integers. 
