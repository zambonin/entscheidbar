...
Now, we turn to the question on whether SHP is decidable. Evidently, the language has a finite number of configurations, due to its limited arithmetic and memory bank. However, a simple program such as \texttt{CALL R0; RET R0} would not be correctly identified as an infinite loop if only these behaviours are taken into account. Hence, one must also pay attention to the recursion states for the program. 
