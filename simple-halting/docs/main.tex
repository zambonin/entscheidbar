...
We can not build a machine $H$, hence $Halt_{TM}$ is undecidable. More precisely, the proof is as follows. Suppose that machine $H$ exists and decides $Halt_{TM}$. Now we introduce a new machine $B$ that takes $M$ and runs $H$ with $<M, <M>>$, and halts if and only if $H$ rejects. 
