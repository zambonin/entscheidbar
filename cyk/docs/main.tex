...
\begin{algorithm}[ht]
  \begin{algorithmic}[1]
      ...
      \FOR{$r \leftarrow 0, \dots, n - \ell - 1$} 
        \FOR{$t \leftarrow 0, \dots, \ell - 1$}
          \STATE{} $\mathcal{L} \leftarrow d_{r}^{r + t}$
            \COMMENT{all prefixes of size $r$ to $r + t$}
          \STATE{} $\mathcal{R} \leftarrow d_{r + t + 1}^{r + \ell}$
            \COMMENT{all suffixes of size $r + t + 1$ to $r + \ell$}
    ...
    \STATE{} $v \leftarrow (S \in d_{0}^{n - 1})$
      \COMMENT{if the starting rule is in the top matrix cell}
  \end{algorithmic}
  \caption{CYK algorithm}\label{alg:cyk}
\end{algorithm}

The algorithm has a complexity of $\mathcal{O}(n^{3} \cdot s)$, $n$ as above and $s$ as the quantity of variables and terminals for every $r \in R$. This happens because, unlike parsers that are restricted to specific types of grammars, CYK can parse every context-free grammar. Further, it was proved by Lee~\cite{} that context-free parsing is equivalent to Boolean matrix multiplication, thus implying that this complexity can be turned sub-cubic.
