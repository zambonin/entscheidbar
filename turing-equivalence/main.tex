...
\end{equation*}
such that only integral solutions are considered, that is, all variables must be integers. Trivial examples are the Pythagorean equation $a^{2} + b^{2} = c^{2}$, or Bézout's identity $ax + by = 1$. Note that variables can appear as exponents, in which case an exponential Diophantine equation occurs. 
