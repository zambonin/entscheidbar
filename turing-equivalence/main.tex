...
Let us formally define a string rewriting system. This definition is taken from~\cite{}. Consider $\Sigma$ to be a finite set of symbols, that is, an alphabet, and a binary relation $R \subseteq \Sigma^{*} \times \Sigma^{*}$. The string rewriting system is the $2$-tuple $S = (\Sigma, R)$. Any member $r \in R$ is called a rewrite rule.
