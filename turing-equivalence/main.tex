...
Note that this computational process may actually never stop, \emph{i.e.} $f(x)$ is an undefined value. It would be interesting to prove that, for every pair of algorithms --- or ``computer programs'' --- and inputs, if the process will halt its execution. 
